...
export default class Vehicle {
    ...
    // R2: Tạo mô hình xe bằng hình học cơ bản - Cải thiện màu sắc và hình dạng
    initGraphics() {
        ...
        // Thân xe chính - màu xanh dương đẹp mắt
        ...
        // Thêm cabin/roof - màu xanh nhạt hơn
        const roofGeometry = new THREE.BoxGeometry(chassisLength * 0.7, chassisHeight * 0.6, chassisWidth * 0.9);
        const roofMaterial = new THREE.MeshPhongMaterial({ 
            color: 0x3399FF,
            shininess: 80
        });
        const roofMesh = new THREE.Mesh(roofGeometry, roofMaterial);
        roofMesh.position.set(0, chassisHeight * 0.3, 0);
        roofMesh.castShadow = true;
        roofMesh.receiveShadow = true;
        this.mesh.add(roofMesh);

        // Thêm spoiler phía sau - màu đen
        const spoilerGeometry = new THREE.BoxGeometry(0.3, 0.2, chassisWidth * 1.1);
        const spoilerMaterial = new THREE.MeshPhongMaterial({ color: 0x1A1A1A });
        const spoilerMesh = new THREE.Mesh(spoilerGeometry, spoilerMaterial);
        spoilerMesh.position.set(chassisLength / 2 - 0.1, chassisHeight * 0.2, 0);
        spoilerMesh.castShadow = true;
        this.mesh.add(spoilerMesh);

        // Bánh xe - cải thiện màu sắc
        this.wheelMeshes = [];
        ...
        wheelPositions.forEach((pos, index) => {
            // Tạo nhóm bánh xe để có thể thêm rim và tire
            ...
            // Rim (vành bánh xe) - nhỏ hơn một chút
            const rimGeometry = new THREE.CylinderGeometry(wheelRadius * 0.6, wheelRadius * 0.6, wheelWidth * 0.3, 16);
    ...
    // Các hàm trợ giúp để lớp RacingEngine lấy thông tin
    getPosition() {
